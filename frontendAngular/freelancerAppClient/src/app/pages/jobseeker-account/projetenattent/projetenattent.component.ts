...
@Component({
  selector: 'app-projetenattent',
  templateUrl: './projetenattent.component.html',
  styleUrls: ['./projetenattent.component.css']
})
export class ProjetenattentComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.userService.getUserInfo().subscribe((data: any) => {
      ...
      this.cvService.getCvId(data.cv.id).subscribe((data: CV) => {
         console.log(data)
        this.propService.getAllPropositionByCvEnattent(data.id).subscribe((data:Proposition[])=>{
          console.log(data)
          this.listprop=data
        })
