...
APP_URL="http://localhost:8080/api/utilisateur/"
  ...
  changeEtat(id:number):Observable<Utilisateur>{
    return this.http.get<Utilisateur>(`${this.APP_URL+"changeEtatUtilisateur/"+id}`)
  }
  saveAdmin(client:Utilisateur):Observable<Utilisateur>{
    return this.http.post<Utilisateur>(`${this.APP_URL+"save/admin"}`,client)
  }
  ...
  getClient():Observable<Utilisateur[]>{
    return this.http.get<Utilisateur[]>(`${this.APP_URL+"client"}`)
  }
  getfreelancer():Observable<Utilisateur[]>{
    return this.http.get<Utilisateur[]>(`${this.APP_URL+"freelancer"}`)
  }
}
