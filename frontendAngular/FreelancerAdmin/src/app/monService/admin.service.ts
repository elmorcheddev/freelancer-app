...
import { Admin } from '../monClass/admin';
...
PATH_APP="http://localhost:8080/auth"
 ...
 public getUser(email:string):Observable<Admin>{
	   return this.httpClient.get<Admin>(`${this.PATH_APP+"/findbyEmail/"+email}`)
   }
 getUserInfo(): Observable<Admin> {
    const token = this.adminAuthService.getToken();   
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
    return this.httpClient.get<Admin>(`${this.PATH_APP+"/user-info"}`, { headers });
  }
}
